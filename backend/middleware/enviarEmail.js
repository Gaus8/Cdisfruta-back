...
export const enviarCorreoRecuperacion = async (usuario, codigo) => {
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.EMAIL_USER,
  ...
  const mailOptions = {
    from: `"CDISFRUTA" <${process.env.EMAIL_USER}>`,
    to: usuario.email,
    subject: `Solicitu de Recuperación de Contraseña - CDISFRUTA`,
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f7f9; padding: 40px 10px; text-align: center;">
        <div style="max-width: 500px; margin: 0 auto; background-color: #ffffff; padding: 30px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
          
          <h1 style="color: #2c3e50; font-size: 24px; margin-bottom: 20px;">Restablecer <span style="color: #ff7961;">Contraseña</span></h1>
          ...
          <p style="color: #555; font-size: 16px; line-height: 1.5; text-align: left;">
            Hemos recibido una solicitud para restablecer la contraseña de tu cuenta en <strong>CDISFRUTA</strong>. Utiliza el siguiente código para continuar:
          </p>

          <div style="margin: 30px 0; padding: 20px; background-color: #fff5f3; border: 2px dashed #ff7961; border-radius: 8px;">
            <span style="font-size: 20px; font-weight: bold; letter-spacing: 8px; color: #ff7961;">
              <a href="${codigo}">Cambiar Contraseña</a>
            </span>
          </div>

          <p style="color: #888; font-size: 14px; margin-top: 25px;">
            Este código vence en <strong>15 minutos</strong>. Si no solicitaste este cambio, puedes ignorar este mensaje de forma segura.
          </p>
